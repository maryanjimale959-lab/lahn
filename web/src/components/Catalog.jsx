...
export function CatalogCard({ item, onError }) {
  ...
  const act = () => {
    ...
    return save(item.url, { kind: item.kind, channel: item.channelId }).catch((err) => onError?.(err.message));
  };

  const label = saved ? t('player.play') : failed ? t('add.retry') : running ? t(STAGE_KEY[job.stage] ?? 'add.downloading') : t('channels.save');

  return (
    <article className={`cat ${saved ? 'saved' : ''} ${running ? 'busy' : ''} ${failed ? 'failed' : ''}`}>
      <button type="button" className="cat-art" onClick={act} title={label} aria-label={`${label} — ${item.title}`}>
        ...
        <span className="cat-fab">
          {saved ? <Icon.play /> : running ? <span className="cat-pct">{Math.round(job.percent ?? 0)}</span> : failed ? <Icon.repeat /> : <Icon.download />}
        </span>
        {running && (
